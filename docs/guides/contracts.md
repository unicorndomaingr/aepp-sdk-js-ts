...
```js
const CONTRACT_SOURCE_CODE = ... // source code of the contract
const contractInstance = await aeSdk.getContractInstance({ source: CONTRACT_SOURCE_CODE.trim() })
// trim function here is a helper to avoid indentation problems when you directly use source code
```
...
- If your contract includes external dependencies which are not part of the [standard library](https://aeternity.com/aesophia/latest/sophia_stdlib) you should initialize the contract using:
  ...
  const contractInstance = await aeSdk.getContractInstance({ source: CONTRACT_SOURCE_CODE, filesystem })
  ```
