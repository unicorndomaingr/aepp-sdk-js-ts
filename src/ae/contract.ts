...
import { BaseAensOptions } from './aens'
...
export abstract class _Contract {
  ...
  async delegateSignatureCommon (
    ids: Array<EncodedData<string>> = [],
    opt: { onAccount?: string | {
  ...
  *  @param args
  ...
  async createAensDelegationSignature (
    { contractId, name }: {contractId: EncodedData<'ct'>, name?: string},
    opt: { onAccount?: {
      publicKey: EncodedData<'ak'>
      secretKey: string
    } } = {}): Promise<string> {
