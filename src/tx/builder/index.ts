...
// INTERFACE
type oracleTxTtlType = typeof TX_TYPE.oracleRegister
  | typeof TX_TYPE.oracleExtend
  | typeof TX_TYPE.oracleQuery
  | typeof TX_TYPE.oracleResponse
...
function getOracleRelativeTtl (params: any, txType: TxType): number {
  ...
  const ttlKey = ttlKeys[txType as oracleTxTtlType]
...
function buildFee (
  ...
  return TX_FEE_BASE_GAS(txType)
    .plus(TX_FEE_OTHER_GAS(txType, txSize, {
      ...
      innerTxSize: txTypes.includes(txType as typeof txTypes[number])
...
export function buildTx<Prefix> (
  ...
  vsn ??= Math.max(
    ...Object.keys(schemas).map(a => +a))
  const schema = schemas[vsn as keyof typeof schemas]
  if (schema == null) {
    throw new SchemaNotFoundError('serialization', type.toString(), vsn)
  }
