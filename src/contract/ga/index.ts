 ...
 */
export async function isGA ({ onAccount }: {onAccount: any}, address: string): Promise<boolean> {
...
export async function createGeneralizedAccount (
  ...
  const contract = await onAccount.getContractInstance({ onAccount, source })

  await contract.compile()
  const { tx }: {tx: string} = await this.buildTx(TX_TYPE.gaAttach, {
    ...opt,
...
export function buildAuthTxHash (
  ...
  return new Uint8Array(hash(
    Buffer.concat([Buffer.from(getNetworkId(onAccount, options)), decode(transaction as EncodedData<'tx'>)])
  ))
}
