...
export abstract class _GeneralizedAccount extends _Contract {
  ...
  async createGeneralizedAccount (
    ...
    const { tx }: {tx: string} = await buildTx(TX_TYPE.gaAttach, {
      ...opt,
  ...
  buildAuthTxHash (
    ...
    return new Uint8Array(hash(
      Buffer.concat([Buffer.from(this.getNetworkId(options)), decode(transaction as EncodedData<'tx'>, 'tx')])
    ))
  }
   ...
   * @returns Account
  */
  abstract getAccount (address: string, { height, hash, onNode }?: {
    height?: number
    hash?: string
    onNode?: string}): Promise<Account>

  abstract gaAttachTx ({ ownerId, gasPrice }: {
    ownerId: string
   ...
   * @returns Transaction
   */
  abstract send (tx: string, options?: any): any
  ...
  abstract prepareTxParams (
    txType: string, ...args: any[]): Promise<{ ttl: number, nonce: number, fee: string | number }>
